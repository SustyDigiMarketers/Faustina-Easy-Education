...
const BlogManagement: React.FC<BlogManagementProps> = ({ posts, onAdd, onUpdate, onDelete }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const postData = { title, category, excerpt, image, content: <p>{excerpt}</p> };
        ...
        <div className="space-y-12">
            <section className="bg-white p-6 sm:p-8 rounded-lg shadow-md">
                ...
                <form ref={formRef} onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                        <label htmlFor="blogExcerpt" className="block text-sm font-medium text-gray-700 mb-1">Excerpt</label>
                        <textarea id="blogExcerpt" value={excerpt} onChange={e => setExcerpt(e.target.value)} required rows={3} className="w-full border-gray-300 rounded-md shadow-sm"></textarea>
                    </div>
                    <div>
