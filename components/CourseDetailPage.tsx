...
const CourseDetailPage: React.FC<CourseDetailPageProps> = ({ course, navigate }) => {
    ...
    const renderRating = (rating: number) => {
        ...
        for (let i = 0; i < fullStars; i++) {
            stars.push(<StarIcon key={`star-${i}`} className="w-5 h-5 text-yellow-400" />);
        }
        ...
        <main className="bg-white">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
                <button
                    ...
                    className="flex items-center text-blue-600 font-semibold hover:text-blue-800 transition-colors mb-8"
                >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 lg:gap-16">
                             ...
                             <ul className="space-y-4">
                                {course.uniqueness.map((feature, index) => (
                                    <li key={index} className="flex items-start">
                                        <CheckIcon className="w-6 h-6 text-green-500 mr-3 mt-1 flex-shrink-0 bg-green-100 p-1 rounded-full" />
                    ...
                    <aside className="lg:sticky lg:top-28 h-fit">
                        <div className="bg-white border border-gray-200 rounded-lg shadow-lg p-6">
                            ...
                            <button className="w-full bg-blue-600 text-white font-bold py-3 px-6 rounded-md hover:bg-blue-700 transition-all duration-300 mb-6">
                                Enroll Now
                            </button>
