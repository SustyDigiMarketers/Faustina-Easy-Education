...
const Header: React.FC<HeaderProps> = ({ navigate, onAdmissionClick, userRole, onLogout }) => {
    ...
    if (userRole) {
        const adminTitle = userRole === 'superadmin' ? 'Superadmin' : 'Admin';
        navItems.push({ name: adminTitle, page: 'admin', href: '#' });
    }
                ...
                <div className="container mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center h-16 sm:h-20">
                    <div className="flex items-center">
                        <a href="#" onClick={(e) => handleNav(e, 'home')} className="flex items-center space-x-2 sm:space-x-3">
                            ...
                            <span className="text-xl sm:text-2xl font-bold text-gray-800">Faustina Easy Education</span>
                        </a>
                    </div>
                    ...
                    <div className="flex items-center space-x-2">
                        ...
                        {userRole && (
                            <button
                                onClick={onLogout}
                                className="bg-red-600 text-white font-bold py-2 px-3 sm:px-5 rounded-md hover:bg-red-700 transition-all duration-300 text-sm sm:text-base"
                            >
                                Logout
                            </button>
                        )}
